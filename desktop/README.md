...
Windows 的窗口与消息循环属于创建它的线程。**从后台线程建窗会得到**：
标题栏写着 `(未响应)`、内容全白、还带着默认的最小化/最大化/关闭按钮。
所以提醒卡的建窗/摆放/显示/关闭**整段都走 `app.run_on_main_thread`**，并带 6 秒超时。
加新窗口时照这个模式抄；**别在普通线程里直接 `WebviewWindowBuilder::build()`**。
