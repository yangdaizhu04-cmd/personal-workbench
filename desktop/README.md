...
## 排障用的三个文件（在 `%APPDATA%\com.personal.workbench\`）

| 文件 | 看什么 |
|---|---|
| `reminders.json` | **最后收到的那份提醒清单**。没有这个文件 = 前端根本没推上来（前端产物没更新？） |
| `fired.log` | 后台线程每发一条通知追加一行：时间戳 \| id \| `ok`/`err` \| 通道（`card` / `system`）或错误 |
| `toast.log` | **提醒卡窗口的真实状态**：`ok \| 标题 \| 装饰=false 376x147 @ 1526,875`。窗口建不出来 / 建歪了 / 没建出来都会在这里留下证据 |

### 建窗口的两条铁律（踩坑 #080 + #081，都栽过）

**铁律一：建窗整段走 `app.run_on_main_thread`。**
Windows 的窗口与消息循环属于创建它的线程。从别的线程建窗会得到标题栏写着 `(未响应)`、
内容全白、还带默认最小化/最大化/关闭按钮的窗口。

**铁律二：派发回主线程之后，绝不要在主线程上等待结果。**
同步命令（`fn xxx()`）是**跑在主线程上**的；如果它 `run_on_main_thread` 之后再阻塞等，
就是"主线程等自己"→ 超时 → 后续代码（比如"9 秒后自动收起"）不执行 →
**窗口永远留在屏幕上，日志还一个字都没有**。正确写法：

```rust
app.run_on_main_thread(move || {
    // 建窗 + 摆放 + 显示 + 写日志，全都放这里；调用方立刻返回
}).map_err(...)?;
// 「自动收起」紧接着就安排，不依赖上面成功与否
std::thread::spawn(move || { sleep(9s); ...close via run_on_main_thread... });
```

配套：**凡是要建窗的命令写 `async fn`**（async 命令跑在异步线程池上，不是主线程）。
验证时**两条路径都要跑**：后台线程（定时提醒）和主线程（设置页自检 / `notify_now`）——
这个 bug 正是"后台那条好、主线程那条炸"。

### 不用手点的自检

```
WB_SELFTEST_NOTIFY=1 启动 exe     # 8 秒后自动发一条提醒（自动化 / 自查用；不设就完全不执行）
```
