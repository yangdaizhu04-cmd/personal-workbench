...
// 基于 ThreeUI Community (MIT) condensationRenderer 重制：色板改为晨雾奶油系的晨露蓝，DPR 上限 1.5
...
export function createCondensationRenderer(
  ...
  const getBubbleSprite = (radius: number) => {
    ...
    spriteContext.fillStyle = `rgba(127,163,189,${0.18 + lit * 0.22})`;
    spriteContext.beginPath();
    spriteContext.arc(center, center, spriteRadius, 0, Math.PI * 2);
    spriteContext.fill();
    spriteContext.strokeStyle = `rgba(110,145,175,${0.35 + lit * 0.35})`;
  ...
  const draw = (now: number) => {
    ...
    splashes.forEach((splash) => {
      ...
      context.strokeStyle = `rgba(127,163,189,${(1 - progress) * 0.52})`;
